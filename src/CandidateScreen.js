...
export default function InterviewerScreen(){
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const snapshot = await get(child(dbRef, 'assignedQuizzes'));
            if (snapshot.exists()) {
                const interviewsData = [];
                snapshot.forEach((childSnapshot) => {
                    const intv=childSnapshot.val();
                    console.log(user.email);
                    if(intv.email===user.email){
                    ...
                    {interviews.map((interview) => (
                        <Card key={interview.id} className="m-2" style={{ width: '18rem' }}>
                            <Card.Body>
                                ...
                                <Button variant="primary" onClick={() => handleCardClick(interview.quiz)}>View Details</Button>
                                <Button style={{marginLeft: '20px'}} variant="success" onClick={() => handleCardClick2(interview.quiz)} className="ml-2">Attempt</Button>
